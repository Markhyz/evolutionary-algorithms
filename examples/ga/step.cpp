...
class StepFunction : public evo_alg::FitnessFunction<double> {
  public:
    StepFunction(size_t n) : FitnessFunction<double>{{n, {-100, 100}}} {};

    size_t getDimension() const override {
        return dimension_;
    }

    StepFunction* clone() const override {
...
evo_alg::real_individual_t polynomialMutation(evo_alg::real_individual_t const& individual, double const pr) {
    return evo_alg::mutator::polynomial(individual, pr, 30);
}
...
int main(int argc, char** argv) {
    ...
    evo_alg::FitnessFunction<double>::const_shared_ptr fit(new StepFunction(n));
